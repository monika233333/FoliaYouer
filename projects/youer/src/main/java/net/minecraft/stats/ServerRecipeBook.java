...
public class ServerRecipeBook extends RecipeBook {
    ...
    public int addRecipes(Collection<RecipeHolder<?>> p_12792_, ServerPlayer p_12793_) {
        ...
        for (RecipeHolder<?> recipeholder : p_12792_) {
            ...
            if (!this.known.contains(resourcelocation) && !recipeholder.value().isSpecial() && CraftEventFactory.handlePlayerRecipeListUpdateEvent(p_12793_, resourcelocation)) { // CraftBukkit
    ...
    private void loadRecipes(ListTag p_12798_, Consumer<RecipeHolder<?>> p_12799_, RecipeManager p_12800_) {
        for (int i = 0; i < p_12798_.size(); i++) {
            ...
            try {
                ...
                if (optional.isEmpty()) {
                    LOGGER.debug("Tried to load unrecognized recipe: {} removed now.", resourcelocation);
                } else {
                    p_12799_.accept(optional.get());
                }
            } catch (ResourceLocationException resourcelocationexception) {
                LOGGER.debug("Tried to load improperly formatted recipe: {} removed now.", s);
            }
        }
    }
