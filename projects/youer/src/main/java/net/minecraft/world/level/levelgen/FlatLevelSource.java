...
public class FlatLevelSource extends ChunkGenerator {
    ...
    @Override
    public ChunkGeneratorStructureState createState(HolderLookup<StructureSet> p_256602_, RandomState p_255830_, long p_256355_) { // Spigot
        ...
        return ChunkGeneratorStructureState.createForFlat(p_255830_, p_256355_, this.biomeSource, stream); // Spigot
    }

    @Override
    public ChunkGeneratorStructureState createStatePaper(HolderLookup<StructureSet> p_256602_, RandomState p_255830_, long p_256355_, org.spigotmc.SpigotWorldConfig conf) { // Spigot
        Stream<Holder<StructureSet>> stream = this.settings
                .structureOverrides()
                .map(HolderSet::stream)
                .orElseGet(() -> p_256602_.listElements().map(p_255579_ -> (Holder<StructureSet>)p_255579_));
        ChunkGeneratorStructureState.injectSpigot$conf.set(conf);
        return ChunkGeneratorStructureState.createForFlat(p_255830_, p_256355_, this.biomeSource, stream).conf(conf); // Spigot
    }
