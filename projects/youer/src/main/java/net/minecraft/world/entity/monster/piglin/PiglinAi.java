...
public class PiglinAi {
    ...
    protected static void pickUpItem(Piglin p_34844_, ItemEntity p_34845_) {
        ...
        ItemStack itemstack;
        // CraftBukkit start
        if (p_34845_.getItem().is(Items.GOLD_NUGGET) && !CraftEventFactory.callEntityPickupItemEvent(p_34844_, p_34845_, 0, false).isCancelled()) {
            p_34844_.take(p_34845_, p_34845_.getItem().getCount());
            itemstack = p_34845_.getItem();
            p_34845_.discard(org.bukkit.event.entity.EntityRemoveEvent.Cause.PICKUP); // CraftBukkit - add Bukkit remove cause
        } else if (!CraftEventFactory.callEntityPickupItemEvent(p_34844_, p_34845_, p_34845_.getItem().getCount() - 1, false).isCancelled()) {
    ...
    protected static void stopHoldingOffHandItem(Piglin p_34868_, boolean p_34869_) {
        ...
        if (p_34868_.isAdult()) {
            ...
            if (p_34869_ && flag) {
                // CraftBukkit start
                org.bukkit.event.entity.PiglinBarterEvent event = CraftEventFactory.callPiglinBarterEvent(p_34868_, getBarterResponseItems(p_34868_), itemstack);
                if (!event.isCancelled()) {
                    throwItems(p_34868_, event.getOutcome().stream().map(CraftItemStack::asNMSCopy).collect(java.util.stream.Collectors.toList()));
                }
                // CraftBukkit end
            } else if (!flag) {
    ...
    private static List<ItemStack> getBarterResponseItems(Piglin p_34997_) {
        ...
        List<ItemStack> list = loottable.getRandomItems(
            new LootParams.Builder((ServerLevel)p_34997_.level())
    ...
    public static void angerNearbyPiglins(Player p_34874_, boolean p_34875_) {
        if (!p_34874_.level().paperConfig().entities.behavior.piglinsGuardChests) return; // Paper - Config option for Piglins guarding chests
        List<Piglin> list = p_34874_.level().getEntitiesOfClass(Piglin.class, p_34874_.getBoundingBox().inflate(16.0));
        list.stream().filter(PiglinAi::isIdle).filter(p_34881_ -> !p_34875_ || BehaviorUtils.canSee(p_34881_, p_34874_)).forEach(p_352819_ -> {
    ...
    protected static void broadcastAngerTarget(AbstractPiglin p_34896_, LivingEntity p_34897_) {
        getAdultPiglins(p_34896_).forEach(p_348314_ -> {
            if (p_34897_.getType() != EntityType.HOGLIN || p_348314_.canHunt() && ((Hoglin)p_34897_).canBeHunted()) {
    ...
    private static void broadcastRetreat(Piglin p_34930_, LivingEntity p_34931_) {
        getVisibleAdultPiglins(p_34930_)
            ...
            .forEach(p_34819_ -> retreatFromNearestTarget((Piglin)p_34819_, p_34931_));
    }
