...
public class Creeper extends Monster implements PowerableMob {
    ...
    protected void dropCustomDeathLoot(ServerLevel p_348566_, DamageSource p_32292_, boolean p_32294_) {
        super.dropCustomDeathLoot(p_348566_, p_32292_, p_32294_);
    ...
    public void explodeCreeper() {
        if (!this.level().isClientSide) {
            ...
            // CraftBukkit start
            org.bukkit.event.entity.ExplosionPrimeEvent event = CraftEventFactory.callExplosionPrimeEvent(this, this.explosionRadius * f, false);
            if (!event.isCancelled()) {
                ...
                this.removeReason(org.bukkit.event.entity.EntityRemoveEvent.Cause.EXPLODE); // CraftBukkit - add Bukkit remove cause
                this.discard();
                // CraftBukkit start
            } else {
                this.swell = 0;
    ...
    private void spawnLingeringCloud() {
        ...
        if (!collection.isEmpty() && !this.level().paperConfig().entities.behavior.disableCreeperLingeringEffect) { // Paper - Option to disable creeper lingering effect
            ...
            areaeffectcloud.spawnReason(org.bukkit.event.entity.CreatureSpawnEvent.SpawnReason.EXPLOSION); // CraftBukkit
    ...
    public void setIgnited(boolean ignited) {
        if (isIgnited() != ignited) {
            CreeperIgniteEvent event = new CreeperIgniteEvent((org.bukkit.entity.Creeper) getBukkitEntity(), ignited);
            if (event.callEvent()) {
