    ...
    public LodestoneTracker tick(ServerLevel p_332798_) {
        if (this.tracked && !this.target.isEmpty()) {
            if (this.target.get().dimension() != p_332798_.dimension()) {
                ...
                net.minecraft.world.level.chunk.LevelChunk chunk = p_332798_.getChunkIfLoaded(blockpos);
                return p_332798_.isInWorldBounds(blockpos) && (chunk == null || chunk.getBlockState(blockpos).getBlock() == net.minecraft.world.level.block.Blocks.LODESTONE) // Paper - Prevent compass from loading chunks
