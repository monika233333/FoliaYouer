...
public class EnderDragon extends Mob implements Enemy {
    ...
    private void knockBack(ServerLevel p_345435_, List<Entity> p_31132_) {
        ...
        for (Entity entity : p_31132_) {
            if (entity instanceof LivingEntity) {
                ...
                entity.pushingEntity = this; // Paper - Add EntityKnockbackByEntityEvent and EntityPushedByEntityAttackEvent
    ...
    private boolean checkWalls(AABB p_31140_) {
                ...
                for (int i2 = k; i2 <= j1; i2++) {
                    ...
                    if (!blockstate.isAir() && !blockstate.is(BlockTags.DRAGON_TRANSPARENT)) {
                        if (net.neoforged.neoforge.common.CommonHooks.canEntityDestroy(this.level(), blockpos, this) && !blockstate.is(BlockTags.DRAGON_IMMUNE)) {
                            ...
                            flag1 = true;
                            destroyedBlocks.add(CraftBlock.at(this.level(), blockpos));
                            // CraftBukkit end
                        } else {
                            flag = true;
                        }
        ...
        org.bukkit.event.entity.EntityExplodeEvent event = CraftEventFactory.callEntityExplodeEvent(this, destroyedBlocks, 0F, explosionSource.getBlockInteraction());
        if (event.isCancelled()) {
    ...
    protected void tickDeath() {
        ...
        if (this.level() instanceof ServerLevel) {
            if (this.dragonDeathTime > 150 && this.dragonDeathTime % 5 == 0 && flag) {
                ...
                ExperienceOrb.award((ServerLevel) this.level(), this.position(), award, org.bukkit.entity.ExperienceOrb.SpawnReason.ENTITY_DEATH, this.lastHurtByPlayer, this); // Paper
            }
    ...
    public float getHeadPartYOffset(int p_31109_, double[] p_31110_, double[] p_31111_) {
        ...
        if (enderdragonphase == EnderDragonPhase.LANDING || enderdragonphase == EnderDragonPhase.TAKEOFF) {
            BlockPos blockpos = this.level().getHeightmapPos(Heightmap.Types.MOTION_BLOCKING_NO_LEAVES,this.getPodium()); // Paper - Allow changing the EnderDragon podium
