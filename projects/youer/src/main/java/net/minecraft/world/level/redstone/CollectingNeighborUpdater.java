...
public class CollectingNeighborUpdater implements NeighborUpdater {
    ...
    public void neighborChanged(BlockState p_230647_, BlockPos p_230648_, Block p_230649_, BlockPos p_230650_, boolean p_230651_) {
        this.addAndRun(
            p_230648_, new CollectingNeighborUpdater.FullNeighborUpdate(p_230647_, p_230648_.immutable(), p_230649_, p_230650_.immutable(), p_230651_)
        );
    }
    ...
    static final class MultiNeighborUpdate implements CollectingNeighborUpdater.NeighborUpdates {
        ...
        public boolean runNext(Level p_230701_) {
            ...
            BlockState blockstate = !ca.spottedleaf.moonrise.common.util.TickThread.isTickThreadFor((ServerLevel)p_230701_, blockpos) ? null : p_230701_.getBlockStateIfLoaded(blockpos); // Folia - prevent block updates in non-loaded or non-owned chunks
            if (blockstate != null) { // Folia - prevent block updates in non-loaded or non-owned chunks
            NeighborUpdater.executeUpdate(p_230701_, blockstate, blockpos, this.sourceBlock, this.sourcePos, false);
            } // Folia - prevent block updates in non-loaded or non-owned chunks
    ...
    static record ShapeUpdate(Direction direction, BlockState state, BlockPos pos, BlockPos neighborPos, int updateFlags, int updateLimit)
        ...
        public boolean runNext(Level p_230716_) {
            if (ca.spottedleaf.moonrise.common.util.TickThread.isTickThreadFor((ServerLevel)p_230716_, this.pos) && p_230716_.getChunkIfLoaded(this.pos) != null) { // Folia - prevent block updates in non-loaded or non-owned chunks
    ...
    static record SimpleNeighborUpdate(BlockPos pos, Block block, BlockPos neighborPos) implements CollectingNeighborUpdater.NeighborUpdates {
        ...
        public boolean runNext(Level p_230734_) {
            BlockState blockstate = !ca.spottedleaf.moonrise.common.util.TickThread.isTickThreadFor((ServerLevel)p_230734_, this.pos) ? null : p_230734_.getBlockStateIfLoaded(this.pos); // Folia - prevent block updates in non-loaded or non-owned chunks
            if (blockstate != null) NeighborUpdater.executeUpdate(p_230734_, blockstate, this.pos, this.block, this.neighborPos, false); // Folia - prevent block updates in non-loaded or non-owned chunks
            return false;
        }
    }
}
