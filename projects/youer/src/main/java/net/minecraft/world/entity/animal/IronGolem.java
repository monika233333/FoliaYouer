...
public class IronGolem extends AbstractGolem implements NeutralMob {
    ...
    protected void doPush(Entity p_28839_) {
        if (p_28839_ instanceof Enemy && !(p_28839_ instanceof Creeper) && this.getRandom().nextInt(20) == 0) {
            this.setTargetReason(org.bukkit.event.entity.EntityTargetLivingEntityEvent.TargetReason.COLLISION, true); // CraftBukkit - set reason
    ...
    public void addAdditionalSaveData(CompoundTag p_28867_) {
        ...
        if (getSummoner() != null) p_28867_.putUUID("Purpur.Summoner", getSummoner()); // Purpur
    ...
    public void readAdditionalSaveData(CompoundTag p_28857_) {
        ...
        if (p_28857_.contains("Purpur.Summoner")) setSummoner(p_28857_.getUUID("Purpur.Summoner")); // Purpur
    ...
    public boolean checkSpawnObstruction(LevelReader p_28853_) {
        ...
        if (!blockstate.entityCanStandOn(p_28853_, blockpos1, this) && !this.level().paperConfig().entities.spawning.ironGolemsCanSpawnInAir) { // Paper - Add option to allow iron golems to spawn in air
            return false;
        } else {
            for (int i = 1; i < 3; i++) {
            ...
            return NaturalSpawner.isValidEmptySpawnBlock(
                    p_28853_, blockpos, p_28853_.getBlockState(blockpos), Fluids.EMPTY.defaultFluidState(), EntityType.IRON_GOLEM
                )
