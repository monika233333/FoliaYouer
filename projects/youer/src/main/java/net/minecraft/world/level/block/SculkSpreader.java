...
public class SculkSpreader {
    ...
    public void load(CompoundTag p_222270_) {
        if (p_222270_.contains("cursors", 9)) {
            ...
            for (int j = 0; j < i; j++) {
                this.addCursor$fireEvent.set(false);
    ...
    public void addCursors(BlockPos p_222267_, int p_222268_) {
        while (p_222268_ > 0) {
            ...
            this.addCursor$fireEvent.set(true);
            this.addCursor(new SculkSpreader.ChargeCursor(p_222267_, i));
            p_222268_ -= i;
        }
    }

    private final AtomicBoolean addCursor$fireEvent = new AtomicBoolean(false);
    private void addCursor(SculkSpreader.ChargeCursor p_222261_) {
        if (this.cursors.size() < 32) {
            // CraftBukkit start
            if (!isWorldGeneration() && addCursor$fireEvent.getAndSet(false)) { // CraftBukkit - SPIGOT-7475: Don't call event during world generation // Paper - add boolean to conditionally fire SculkBloomEvent
