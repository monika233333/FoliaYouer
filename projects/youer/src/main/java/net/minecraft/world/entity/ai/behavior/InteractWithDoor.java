...
public class InteractWithDoor {
                                ...
                                if (!path.notStarted() && !path.isDone()) {
                                    ...
                                    if (blockstate.is(BlockTags.MOB_INTERACTABLE_DOORS, p_201959_ -> p_201959_.getBlock() instanceof DoorBlock)) {
                                        ...
                                        if (!doorblock.isOpen(blockstate)) {
                                            ...
                                            org.bukkit.event.entity.EntityInteractEvent event = new org.bukkit.event.entity.EntityInteractEvent(p_258470_.getBukkitEntity(), CraftBlock.at(p_258470_.level(), blockpos));
                                    ...
                                    if (blockstate1.is(BlockTags.MOB_INTERACTABLE_DOORS, p_201957_ -> p_201957_.getBlock() instanceof DoorBlock)) {
                                        ...
                                        if (!doorblock1.isOpen(blockstate1)) {
                                            // CraftBukkit start - entities opening doors
                                            org.bukkit.event.entity.EntityInteractEvent event = new org.bukkit.event.entity.EntityInteractEvent(p_258470_.getBukkitEntity(), CraftBlock.at(p_258470_.level(), blockpos1));
                                            p_258470_.level().getCraftServer().getPluginManager().callEvent(event);
                                            if (event.isCancelled()) {
                                                return false;
                                            }
    ...
    private static boolean areOtherMobsComingThroughDoor(LivingEntity p_260091_, BlockPos p_259764_, Optional<List<LivingEntity>> p_259365_) {
        return p_259365_.isEmpty()
            ...
            : p_259365_.get()
                .stream()
                .filter(p_348184_ -> p_348184_.getType() == p_260091_.getType())
                .filter(p_352722_ -> p_259764_.closerToCenterThan(p_352722_.position(), 2.0))
