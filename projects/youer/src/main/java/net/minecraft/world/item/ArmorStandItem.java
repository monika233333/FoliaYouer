        ...
        if (direction == Direction.DOWN) {
            ...
            if (level.noCollision(null, aabb) && level.getEntities(null, aabb).isEmpty()) {
                if (level instanceof ServerLevel serverlevel) {
                    ...
                    // CraftBukkit start
                    if (CraftEventFactory.callEntityPlaceEvent(p_40510_, armorstand).isCancelled()) {
                        if (p_40510_.getPlayer() != null) p_40510_.getPlayer().containerMenu.sendAllDataToRemote(); // Paper - Fix inventory desync
                        return InteractionResult.FAIL;
                    }
                    // CraftBukkit end
