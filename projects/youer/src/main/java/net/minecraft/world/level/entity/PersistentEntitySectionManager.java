...
public class PersistentEntitySectionManager<T extends EntityAccess> implements AutoCloseable {
    ...
    private boolean addEntityUuid(T p_157558_) {
        if (!this.knownUuids.add(p_157558_.getUUID())) {
            if (false) LOGGER.warn("UUID of added entity already exists: {}", p_157558_);
            return false;
        } else {
            return true;
        }
    ...
    private boolean addEntity(T p_157539_, boolean p_157540_) {
        if (p_157539_ instanceof Entity entity && (EntityLimitsConfig.INSTANCE.canSpawn(entity) || BanEntity.check(entity) || net.neoforged.neoforge.common.NeoForge.EVENT_BUS.post(new net.neoforged.neoforge.event.entity.EntityJoinLevelEvent(entity, entity.level(), p_157540_)).isCanceled())) return false;
        ...
        if (persistententitysectionmanager$chunkloadstatus == PersistentEntitySectionManager.ChunkLoadStatus.PENDING) {
            ...
            if (list.isEmpty()) {
                if (persistententitysectionmanager$chunkloadstatus == PersistentEntitySectionManager.ChunkLoadStatus.LOADED) {
                    if (!AsyncCatcher.catchAsync() && callEvent) CraftEventFactory.callEntitiesUnloadEvent(((EntityStorage) permanentStorage).level, new ChunkPos(p_157513_), ImmutableList.of()); // CraftBukkit
                ...
                if (!AsyncCatcher.catchAsync() && callEvent) CraftEventFactory.callEntitiesUnloadEvent(((EntityStorage) permanentStorage).level, new ChunkPos(p_157513_), list.stream().map(entity -> (Entity) entity).collect(Collectors.toList())); // CraftBukkit
    ...
    private boolean processChunkUnload(long p_157569_) {
        boolean flag = this.storeChunkSections(p_157569_, p_157595_ -> p_157595_.getPassengersAndSelf().forEach(this::unloadEntity), true); // CraftBukkit - add boolean for event call
        if (!flag) {
            return false;
        } else {
    ...
    private void unloadEntity(EntityAccess p_157586_) {
        ((Entity)p_157586_).removeReason(EntityRemoveEvent.Cause.UNLOAD); // CraftBukkit - add Bukkit remove cause
