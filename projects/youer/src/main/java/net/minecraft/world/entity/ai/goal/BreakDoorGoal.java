...
public class BreakDoorGoal extends DoorInteractGoal {
    ...
    public boolean canUse() {
        if (!super.canUse()) {
            ...
            return !net.neoforged.neoforge.common.CommonHooks.canEntityDestroy(this.mob.level(), this.doorPos, this.mob)
                ? false
    ...
    public void tick() {
        ...
        if (this.breakTime == this.getDoorBreakTime() && this.isValidDifficulty(this.mob.level().getDifficulty())) {
            // CraftBukkit start
            if (CraftEventFactory.callEntityBreakDoorEvent(this.mob, this.doorPos, this.mob.level().getFluidState(this.doorPos).createLegacyBlock()).isCancelled()) {
                this.start();
                return;
            }
            // CraftBukkit end
