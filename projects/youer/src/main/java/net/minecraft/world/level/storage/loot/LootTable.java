...
public class LootTable {
    ...
    public static Consumer<ItemStack> createStackSplitter(ServerLevel p_287765_, Consumer<ItemStack> p_251308_) {
        boolean skipSplitter = p_287765_ != null && !p_287765_.paperConfig().fixes.splitOverstackedLoot; // Paper - preserve overstacked items
        return p_287570_ -> {
            if (p_287570_.isItemEnabled(p_287765_.enabledFeatures())) {
                if (skipSplitter || p_287570_.getCount() < p_287570_.getMaxStackSize()) {
    ...
    public AtomicBoolean fill$plugin = new AtomicBoolean(false);
    public void fill(Container p_287662_, LootParams p_287743_, long p_287585_) {
        ...
        // CraftBukkit start
        if (craftLootTable != null && lootcontext.hasParam(LootContextParams.ORIGIN) && lootcontext.hasParam(LootContextParams.THIS_ENTITY)) {
            LootGenerateEvent event = CraftEventFactory.callLootGenerateEvent(p_287662_, this, lootcontext, objectarraylist, fill$plugin.getAndSet(false));
