...
public class SonicBoom extends Behavior<Warden> {
    ...
    protected void tick(ServerLevel p_217724_, Warden p_217725_, long p_217726_) {
        p_217725_.getBrain().getMemory(MemoryModuleType.ATTACK_TARGET).ifPresent(p_352767_ -> p_217725_.getLookControl().setLookAt(p_352767_.position()));
        if (!p_217725_.getBrain().hasMemoryValue(MemoryModuleType.SONIC_BOOM_SOUND_DELAY)
            && !p_217725_.getBrain().hasMemoryValue(MemoryModuleType.SONIC_BOOM_SOUND_COOLDOWN)) {
            ...
            p_217725_.getBrain()
                ...
                .ifPresent(p_340767_ -> {
                    ...
                    if (p_340767_.hurt(p_217724_.damageSources().sonicBoom(p_217725_), 10.0F)) {
                        ...
                        p_340767_.pushingEntity = p_217725_; // Paper - Add EntityKnockbackByEntityEvent and EntityPushedByEntityAttackEvent
