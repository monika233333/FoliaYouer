...
public class ServerChunkCache extends ChunkSource implements net.neoforged.neoforge.common.extensions.IServerChunkCacheExtension {
    ...
    public final ServerChunkCache.MainThreadExecutor mainThreadProcessor; // Folia - region threading - public
    ...
    public LevelChunk getChunkAtIfLoadedImmediately(int x, int z) {
        ChunkHolder chunk = this.chunkMap.getUpdatingChunkIfPresent(ChunkPos.asLong(x, z));
        if (chunk == null) {
            return null;
        }
        return chunk.getFullChunkNow() != null ? chunk.getFullChunkNow() : null;
    }
    ...
    public CompletableFuture<ChunkResult<ChunkAccess>> getChunkFuture(int p_8432_, int p_8433_, ChunkStatus p_330326_, boolean p_8435_) {
        ...
        if (flag) {
            completablefuture = this.getChunkFutureMainThread(p_8432_, p_8433_, p_330326_, p_8435_);
            this.mainThreadProcessor.managedBlock(completablefuture::isDone);
        } else {
            completablefuture = CompletableFuture.<CompletableFuture<ChunkResult<ChunkAccess>>>supplyAsync(
                    () -> this.getChunkFutureMainThread(p_8432_, p_8433_, p_330326_, p_8435_), this.mainThreadProcessor
                )
    ...
    private boolean chunkAbsent(@Nullable ChunkHolder p_8417_, int p_8418_) {
        return p_8417_ == null || p_8417_.oldTicketLevel > p_8418_ || p_8417_.getTicketLevel() > p_8418_; // CraftBukkit using oldTicketLevel for isLoaded checks
    }
    ...
    public boolean hasChunk(int p_8429_, int p_8430_) {
        ...
        // Non-blocking check: the chunk is truly loaded at FULL only if the FULL chunk is
        // available in memory or currently being post-loaded (currentlyLoading).
        // Ticket-level checks can race and return true for chunks that are not yet at FULL,
        // causing re-entrant chunk loads that deadlock inside ChunkTaskPriorityQueueSorter.
        return chunkholder.getChunkIfPresent(ChunkStatus.FULL) != null || chunkholder.currentlyLoading != null;
    }
    ...
    // Paper start
    public boolean isPositionTickingPaper(Entity entity) {
    ...
    // Folia start - region threading - addTicketAtLevel/removeTicketAtLevel delegates
        ...
        if (!this.level.isDebug()) {
            ...
            if (this.level.tickRateManager().runsNormally()) {
                ...
                for (ServerChunkCache.ChunkAndHolder serverchunkcache$chunkandholder : list) {
                    ...
                    this.chunkMap.reducedRange.set(true); // Youer
    ...
    public static record ChunkAndHolder(LevelChunk chunk, ChunkHolder holder) { // Folia - region threading - public
    }

    public final class MainThreadExecutor extends BlockableEventLoop<Runnable> { // Folia - region threading - public
