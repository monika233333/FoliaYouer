...
public class ServerPacksSource extends BuiltInPackSource {
    ...
    protected Pack createBuiltinPack(String p_250596_, Pack.ResourcesSupplier p_249625_, Component p_249043_) {
        ...
        if ("paper".equals(p_250596_)) {
            info = new PackLocationInfo(p_250596_, p_249043_, PackSource.BUILT_IN, Optional.empty());
            packConfig = new PackSelectionConfig(true, Pack.Position.TOP, true);
        } else {
            info = createBuiltInPackLocation(p_250596_, p_249043_);
            packConfig = FEATURE_SELECTION_CONFIG;
        }
        return Pack.readMetaAndCreate(info, p_249625_, PackType.SERVER_DATA, packConfig);
